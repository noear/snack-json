...
public class OffsetDateTimeDecoder implements ObjectDecoder<OffsetDateTime> {
    @Override
    public OffsetDateTime decode(Options opts, ONodeAttr attr, ONode node, Class<?> clazz) {
        ZoneId zoneId = opts.getTimeZone().toZoneId();
        if (attr != null && Asserts.isNotEmpty(attr.timezone())) {
            zoneId = ZoneId.of(attr.timezone());
        }

        return DateUtil.decode(opts, attr, node, clazz).atZone(zoneId).toOffsetDateTime();
    }
}
